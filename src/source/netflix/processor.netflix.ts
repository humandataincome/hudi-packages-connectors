...
export class ProcessorNetflix {
    ...
    ): Promise<NetflixDataAggregator | undefined> {
        try {
    ...
    static async aggregatorBuilder(
        data: Buffer,
        pathName: string,
        model: NetflixDataAggregator,
        options: ProcessorOptions = {},
    ) {
        let result;
        let regex;
        if (
            (regex = new RegExp(FileCodeNetflix.ACCOUNT_DETAILS)) &&
            regex.test(pathName)
        ) {
            result = await ServiceNetflix.parsePersonalInformation(data);
            result && (model.accountInfo = result);
        } else if (
            (regex = new RegExp(FileCodeNetflix.PROFILES)) &&
            regex.test(pathName)
        ) {
            ...
            (regex = new RegExp(FileCodeNetflix.CONTENT_INTERACTION_MY_LIST)) &&
            regex.test(pathName)
        ) {
