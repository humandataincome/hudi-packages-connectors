        ...
        try {
            ...
            if (jsonFormat) {
                ...
                if (document && document[0] && document[0].account) {
                    ...
                    document.createdVia &&
                        (model.createdVia = document.createdVia);
                    document.username && (model.username = document.username);
                    document.accountId &&
                        (model.accountId = document.accountId);
                    document.createdAt &&
                        (model.createdAt = new Date(document.createdAt));
                    document.accountDisplayName &&
                ...
                throw Error(
                    "It's not possible to extract a JSON from the given JS file",
                );
            }
        } catch (error) {
