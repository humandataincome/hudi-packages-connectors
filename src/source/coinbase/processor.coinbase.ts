...
export class ProcessorCoinbase {
    ...
    static async aggregatorBuilder(
        ...
        try {
            ...
            if (model.accounts) {
                ...
                const currenciesAnalyzed: string[] = [];
                ...
                await Promise.all(
                    model.accounts.list.map(async (account: AccountCB) => {
                        if (account.currency) {
                            if (
                                (account.balance &&
                                ...
                                (currencies.includes(account.currency) &&
                                    !currenciesAnalyzed.includes(
                                        account.currency,
                                    ))
                            ) {
                                if (
                                    await this.buildMovementsAggregator(
                                        account,
                                        model,
                                        service,
                                    )
    ...
    private static async buildMovementsAggregator(
        account: AccountCB,
        model: CoinbaseDataAggregator,
        service: ServiceCoinbase,
    ) {
