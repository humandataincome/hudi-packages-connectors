...
export class ServiceCoinbase {
    ...
    async getUserAPI(): Promise<UserCB | undefined> {
        try {
            let response = await this.httpMethod(
                this.getConfig('GET', '/v2/user'),
            );
            const model: UserCB = {};
            if (response.data) {
                ...
                response.profile_location &&
                    (model.profileLocation = response.profile_location);
                response.profile_bio &&
                    (model.profileBio = response.profile_bio);
                response.profile_url &&
                    (model.profileUrl = response.profile_url);
                ...
                response.resource_path &&
                    (model.resourcePath = response.resource_path);
                ...
                response.native_currency &&
                    (model.nativeCurrency = response.native_currency);
                response.bitcoin_unit &&
                    (model.bitcoinUnit = response.bitcoin_unit);
                ...
                response.region_supports_fiat_transfers !== undefined &&
                    (model.regionSupportsFiatTransfers =
                        response.region_supports_fiat_transfers);
                response.region_supports_crypto_to_crypto_transfers !==
                    undefined &&
                    (model.regionSupportsCryptoToCryptoTransfers =
                        response.region_supports_crypto_to_crypto_transfers);
                response.created_at &&
                    (model.createdAt = new Date(response.created_at));
                response.supports_rewards !== undefined &&
                    (model.supportsRewards = response.supports_rewards);
                response.has_blocking_buy_restrictions !== undefined &&
                    (model.hasBlockingBuyRestrictions =
                        response.has_blocking_buy_restrictions);
                response.has_made_a_purchase !== undefined &&
                    (model.hasMadePurchase = response.has_made_a_purchase);
                response.has_buy_deposit_payment_methods !== undefined &&
                    (model.hasBuyDepositPaymentMethods =
                        response.has_buy_deposit_payment_methods);
                response.has_unverified_buy_deposit_payment_methods !==
                    undefined &&
                    (model.hasUnverifiedBuyDepositPaymentMethods =
                        response.has_unverified_buy_deposit_payment_methods);
                response.needs_kyc_remediation !== undefined &&
                    (model.needsKycRemediation =
                        response.needs_kyc_remediation);
                response.show_instant_ach_ux !== undefined &&
                    (model.showInstantAchUx = response.show_instant_ach_ux);
    ...
    async getAccountsAPI(): Promise<AccountsCB | undefined> {
        try {
            ...
            while (nextURI) {
                const response: any = await this.httpMethod(
                    this.getConfig('GET', nextURI),
                );
                if (response) {
                    response.data &&
                        (model.list = model.list.concat(
                            response.data.map((account: any) => {
                                ...
                                account.primary !== undefined &&
                                    (subModel.primary = account.primary);
                                account.type && (subModel.type = account.type);
                                account.currency &&
                                    (subModel.currency = account.currency);
                                account.resource &&
                                    (subModel.resource = account.resource);
                                account.resource_path &&
                                    (subModel.resourcePath =
                                        account.resource_path);
                                account.allow_deposits !== undefined &&
                                    (subModel.allowDeposits =
                                        account.allow_deposits);
                                account.allow_withdrawals !== undefined &&
                                    (subModel.allowWithdrawals =
                                        account.allow_withdrawals);
                                account.created_at &&
                                    (subModel.createdAt = new Date(
                                        account.created_at,
                                    ));
                                account.updated_at &&
                                    (subModel.updatedAt = new Date(
                                        account.updated_at,
                                    ));
                                account.balance &&
                                    (subModel.balance = this.buildBalance(
                                        account.balance,
                                    ));
                                account.native_balance &&
                                    (subModel.nativeBalance = this.buildBalance(
                                        account.native_balance,
                                    ));
                                return !ValidatorObject.objectIsEmpty(subModel)
                                    ? subModel
                                    : undefined;
                            }),
                        ));
    ...
    async getTransactionsAPI(
        accountId: string,
    ): Promise<TransactionsCB | undefined> {
        try {
            ...
            while (nextURI) {
                ...
                if (response) {
                    response.data &&
                        (model.list = model.list.concat(
                            response.data.map((account: any) => {
                                ...
                                account.type && (subModel.type = account.type);
                                account.status &&
                                    (subModel.status = account.status);
                                account.amount &&
                                    (subModel.amount = this.buildBalance(
                                        account.amount,
                                    ));
                                account.native_amount &&
                                    (subModel.nativeAmount = this.buildBalance(
                                        account.native_amount,
                                    ));
                                account.description &&
                                ...
                                account.resourcePath &&
                                    (subModel.resourcePath =
                                        account.resourcePath);
                                account.instantExchange !== undefined &&
                                    (subModel.instantExchange =
                                        account.instantExchange);
                                account.idem && (subModel.idem = account.idem);
                                account.hideNativeAmount !== undefined &&
                                    (subModel.hideNativeAmount =
                                        account.hideNativeAmount);
                                account.network &&
                                    (subModel.network = this.buildNetwork(
                                        account.network,
                                    ));
                                account.to &&
                                    (subModel.to = this.buildTo(account.to));
                                account.application &&
                                    (subModel.application = this.buildResource(
                                        account.application,
                                    ));
                                account.details &&
                                    (subModel.details = this.buildDetails(
                                        account.details,
                                    ));
                                return !ValidatorObject.objectIsEmpty(subModel)
                                    ? subModel
                    ...
                    response.pagination && response.pagination.next_uri
                        ? (nextURI = response.pagination.next_uri)
                        : (nextURI = undefined);
                } else {
                    nextURI = undefined;
    ...
    async getDepositHistoryAPI(
        accountId: string,
    ): Promise<MovementsCB | undefined> {
        try {
            let nextURI: string | undefined =
                `/v2/accounts/${accountId}/deposits`;
            ...
            while (nextURI) {
                ...
                if (response) {
                    response.data &&
                        (model.list = response.data.map((deposit: any) =>
                            this.buildCurrencyMovement(deposit),
                        ));
                    response.pagination && response.pagination.next_uri
                        ? (nextURI = response.pagination.next_uri)
                        : (nextURI = undefined);
    ...
    async getWithdrawHistoryAPI(
        ...
        try {
            let nextURI: string | undefined =
                `/v2/accounts/${accountId}/withdrawals`;
            const model: MovementsCB = { list: [] };
            while (nextURI) {
                const response: any = await this.httpMethod(
                    this.getConfig('GET', nextURI),
                ...
                if (response) {
                    response.data &&
                        (model.list = response.data.map((withdraw: any) =>
                            this.buildCurrencyMovement(withdraw),
                        ));
                    response.pagination && response.pagination.next_uri
                        ? (nextURI = response.pagination.next_uri)
                        : (nextURI = undefined);
    ...
    async getBuysAPI(accountId: string): Promise<MovementsCB | undefined> {
        try {
            ...
            while (nextURI) {
                ...
                if (response) {
                    response.data &&
                        (model.list = response.data.map((buys: any) =>
                            this.buildCurrencyMovement(buys),
                        ));
                    response.pagination && response.pagination.next_uri
                        ? (nextURI = response.pagination.next_uri)
                        : (nextURI = undefined);
    ...
    async getSellsAPI(accountId: string): Promise<MovementsCB | undefined> {
        try {
            ...
            while (nextURI) {
                ...
                if (response) {
                    response.data &&
                        (model.list = response.data.map((sells: any) =>
                            this.buildCurrencyMovement(sells),
                        ));
                    response.pagination && response.pagination.next_uri
                        ? (nextURI = response.pagination.next_uri)
                        : (nextURI = undefined);
    ...
    private buildCurrencyMovement(document: any) {
        ...
        document.transaction &&
            (subModel.transaction = this.buildResource(document.transaction));
        document.user_reference &&
            (subModel.userReference = document.user_reference);
        document.created_at &&
            (subModel.createdAt = new Date(document.created_at));
        document.updated_at &&
            (subModel.updatedAt = new Date(document.updated_at));
        document.resource && (subModel.resource = document.resource);
        document.resource_path &&
            (subModel.resourcePath = document.resource_path);
        document.payment_method &&
            (subModel.paymentMethod = this.buildResource(
                document.payment_method,
            ));
        document.committed !== undefined &&
            (subModel.committed = document.committed);
        document.payout_at &&
            (subModel.payoutAt = new Date(document.payout_at));
        document.instant !== undefined && (subModel.instant = document.instant);
        document.fees && (subModel.fees = document.fees);
        document.amount &&
            (subModel.amount = this.buildBalance(document.amount));
        document.subtotal &&
            (subModel.subtotal = this.buildBalance(document.subtotal));
        ...
        document.fee_explanation_url &&
            (subModel.feeExplanationUrl = document.fee_explanation_url);
        return subModel;
    }
    ...
    private buildResource(document: any) {
        ...
        document.resource_path &&
            (transaction.resourcePath = document.resource_path);
        return transaction;
    }
    ...
    private buildNetwork(document: any) {
        ...
        document.status_description &&
            (model.statusDescription = document.status_description);
        document.hash && (model.hash = document.hash);
        document.transaction_url &&
            (model.transactionUrl = document.transaction_url);
        document.transaction_fee &&
            (model.transactionFee = this.buildBalance(
                document.transaction_fee,
            ));
        document.transaction_amount &&
            (model.transactionAmount = this.buildBalance(
                document.transaction_amount,
            ));
        document.confirmations &&
            (model.confirmations = document.confirmations);
        return model;
    }
    ...
    private buildTo(document: any) {
        ...
        document.address_info &&
            (model.addressInfo = this.buildAddress(document.address_info));
