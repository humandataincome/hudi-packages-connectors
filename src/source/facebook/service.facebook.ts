...
export class ServiceFacebook {
    ...
    static async parsePersonalInformation(data: Buffer): Promise<PersonalInformationFB | undefined> {
        try {
            ...
            if (document) {
                (document.profile_v2?.name?.first_name) && (personalInfoModel.firstName = DecodingUtils.decodeObject(document.profile_v2.name.first_name));
                (document.profile_v2?.name?.middle_name) && (personalInfoModel.middleName = DecodingUtils.decodeObject(document.profile_v2.name.middle_name));
                (document.profile_v2?.name?.last_name) && (personalInfoModel.lastName = DecodingUtils.decodeObject(document.profile_v2.name.last_name));
                (document.profile_v2?.emails?.emails) && (personalInfoModel.emails = DecodingUtils.decodeObject(document.profile_v2.emails.emails));
                ...
                (document.profile_v2?.gender?.gender_option) && (personalInfoModel.gender = DecodingUtils.decodeObject(document.profile_v2.gender.gender_option));
                (document.profile_v2?.current_city?.name) && (personalInfoModel.currentCity = DecodingUtils.decodeObject(document.profile_v2.current_city.name));
                (document.profile_v2?.hometown?.name) && (personalInfoModel.homeTown = DecodingUtils.decodeObject(document.profile_v2.hometown.name));

                const relationshipModel: RelationshipFB = {};
                (document.profile_v2?.relationship?.status) && (relationshipModel.status = DecodingUtils.decodeObject(document.profile_v2.relationship.status));
                ...
                (document.profile_v2?.education_experiences?.length > 0) && (personalInfoModel.educationExperiences =
                    document.profile_v2.education_experiences.map((value: any) => {
                        ...
                        (value.name) && (educationModel.name = DecodingUtils.decodeObject(value.name));
                        ...
                        (value.description) && (educationModel.description = DecodingUtils.decodeObject(value.description));
                        (value.concentrations && value.concentrations.length > 0) && (educationModel.educationTopics = DecodingUtils.decodeObject(value.concentrations));
                        (value.degree) && (educationModel.degree = DecodingUtils.decodeObject(value.degree));
                        (value.school_type) && (educationModel.schoolType = DecodingUtils.decodeObject(value.school_type));
                ...
                (document.profile_v2?.work_experiences?.length > 0) && (personalInfoModel.workExperience =
                    document.profile_v2.work_experiences.map((value: any) => {
                        ...
                        (value.employer) && (workModel.employer = DecodingUtils.decodeObject(value.employer));
                        (value.title) && (workModel.title = DecodingUtils.decodeObject(value.title));
                        (value.location) && (workModel.location = DecodingUtils.decodeObject(value.location));
                        (value.description) && (workModel.description = DecodingUtils.decodeObject(value.description));
                ...
                (document.profile_v2?.languages?.length > 0) && (personalInfoModel.languages = document.profile_v2.languages.map((value: any) => DecodingUtils.decodeObject(value.name)));
                (document.profile_v2?.interested_in?.length > 0) && (personalInfoModel.gendersInterests = DecodingUtils.decodeObject(document.profile_v2.interested_in));
                (document.profile_v2?.political_view?.length > 0) && (personalInfoModel.politicalView = document.profile_v2.political_view.map((value: any) => {
                    ...
                    (value.name) && (viewModel.name = DecodingUtils.decodeObject(value.name));
                    (value.description) && (viewModel.description = DecodingUtils.decodeObject(value.description));
                    return viewModel;
                }));
                ...
                (document.profile_v2?.blood_donor_status) && (personalInfoModel.bloodInfo = DecodingUtils.decodeObject(document.profile_v2.blood_info.blood_donor_status));
                (document.profile_v2?.websites && document.profile_v2.websites.length > 0) && (personalInfoModel.websites = document.profile_v2.websites.map((value: any) => DecodingUtils.decodeObject(value.address)));

                const addressModel: AddressLocationFB = {};
                (document.profile_v2?.address?.street) && (addressModel.street = DecodingUtils.decodeObject(document.profile_v2.address.street));
                (document.profile_v2?.address?.city) && (addressModel.city = DecodingUtils.decodeObject(document.profile_v2.address.city));
                (document.profile_v2?.address?.zipcode) && (addressModel.zipcode = DecodingUtils.decodeObject(document.profile_v2.address.zipcode));
                (document.profile_v2?.address?.neighborhood) && (addressModel.neighborhood = DecodingUtils.decodeObject(document.profile_v2.address.neighborhood));
                (document.profile_v2?.address?.country) && (addressModel.country = DecodingUtils.decodeObject(document.profile_v2.address.country));
                (document.profile_v2?.address?.country_code) && (addressModel.countryCode = DecodingUtils.decodeObject(document.profile_v2.address.country_code));
                (document.profile_v2?.address?.region) && (addressModel.region = DecodingUtils.decodeObject(document.profile_v2.address.region));
                ...
                (document.profile_v2?.places_lived?.length > 0) && (personalInfoModel.placesLived = document.profile_v2.places_lived.map((value: any) => {
                    ...
                    (value.place) && (placeModel.place = DecodingUtils.decodeObject(value.place));
                ...
                (document.profile_v2?.pages?.length > 0) && (personalInfoModel.pagesInterests =
                    document.profile_v2.pages.map((value: any) => {
                        ...
                        (value.name) && (pagesModel.category = DecodingUtils.decodeObject(value.name));
                        (value.pages && value.pages.length > 0) && (pagesModel.pages = DecodingUtils.decodeObject(value.pages));
                        return pagesModel;
                    }));
                ...
                (document.profile_v2?.profile_uri) && (personalInfoModel.profileUri = DecodingUtils.decodeObject(document.profile_v2.profile_uri));
            }
    ...
    static async parseAdsInteractedWith(data: Buffer): Promise<AdsInteractedWithFB | undefined> {
        try {
            ...
            (document && document.history_v2 && document.history_v2.length > 0) && (adsModel.list = document.history_v2.map((value: any) => {
                ...
                (value.title) && (model.title = DecodingUtils.decodeObject(value.title));
                (value.action) && (model.action = DecodingUtils.decodeObject(value.action));
    ...
    static async parseAdsUsingYourInfo(data: Buffer): Promise<AdsUsingYourInfoFB | undefined> {
        try {
            ...
            (document && document.custom_audiences_all_types_v2 && document.custom_audiences_all_types_v2.length > 0) && (adsModel.list = document.custom_audiences_all_types_v2.map((value: any) => {
                ...
                (value.advertiser_name) && (model.advertiserName = DecodingUtils.decodeObject(value.advertiser_name));
    ...
    static async parseSearchHistory(data: Buffer): Promise<SearchHistoryFB | undefined> {
        try {
            ...
            (document && document.searches_v2 && document.searches_v2.length > 0) && (searchHistoryModel.list = document.searches_v2.map((value: any) => {
                ...
                (value.data && value.data[0].text) && (model.text = DecodingUtils.decodeObject(value.data[0].text));
                (value.timestamp) && (model.date = new Date(1000*value.timestamp));
                return model;
            }));
    ...
    static async parseComments(data: Buffer): Promise<CommentsPostedFB | undefined> {
        try {
            ...
            (document && document.comments_v2 && document.comments_v2.length > 0) && (commentsPostedModel.list = document.comments_v2.map((value: any) => {
                ...
                (value.data && value.data[0].comment?.comment) && (model.text = DecodingUtils.decodeObject(value.data[0].comment.comment));
                (value.data && value.data[0].comment?.author) && (model.author = DecodingUtils.decodeObject(value.data[0].comment.author));
                (value.timestamp) && (model.date = new Date(1000 * value.timestamp));
                (value.title) && (model.title = DecodingUtils.decodeObject(value.title));
                return model;
            }));
    ...
    static async parseReactions(data: Buffer): Promise<ReactionsFB | undefined> {
        try {
            ...
            (document && document.reactions_v2 && document.reactions_v2.length > 0) && (reactionsModel.list = document.reactions_v2.map((value: any) => {
                ...
                (value.data && value.data[0].reaction?.reaction) && (model.reaction = DecodingUtils.decodeObject(value.data[0].reaction.reaction));
                (value.data && value.data[0].reaction?.actor) && (model.actor = DecodingUtils.decodeObject(value.data[0].reaction.actor));
                (value.timestamp) && (model.date = new Date(1000 * value.timestamp));
                (value.title) && (model.title = DecodingUtils.decodeObject(value.title));
                return model;
            }));
    ...
    static async parsePagesLiked(data: Buffer): Promise<PagesLikedFB | undefined> {
        try {
            ...
            (document && document.page_likes_v2 && document.page_likes_v2.length > 0) && (modelPagesLiked.list = document.page_likes_v2.map((value: any) => {
                const model: PageFB = {};
                (value.name) && (model.name = DecodingUtils.decodeObject(value.name));
                (value.timestamp) && (model.date = new Date(1000 * value.timestamp));
                return model;
            }));
    ...
    static async parsePagesFollowed(data: Buffer): Promise<PagesFollowFB | undefined> {
        try {
            ...
            (document && document.pages_followed_v2 && document.pages_followed_v2.length > 0) && (modelPagesFollow.list = document.pages_followed_v2.map((value: any) => {
                const model: PageFB = {};
                (value.title) && (model.name = DecodingUtils.decodeObject(value.title));
                (value.timestamp) && (model.date = new Date(1000 * value.timestamp));
                return model;
            }));
    ...
    static async parseAppsConnected(data: Buffer): Promise<AppsConnectedFB | undefined> {
        try {
            ...
            (document) && (modelAppsConnected.list = document.installed_apps_v2.map((value: any) => {
                ...
                (value.category) && (model.category = DecodingUtils.decodeObject(value.category));
    ...
    static async parseLanguages(data: Buffer): Promise<LanguagesFB | undefined> {
        try{
            ...
            if (document && document.language_and_locale_v2 && document.language_and_locale_v2.length > 0) {
                (document.language_and_locale_v2[0].children[0].entries[0].data?.value) && (model.settingsLanguage = DecodingUtils.decodeObject(document.language_and_locale_v2[0].children[0].entries[0].data.value));
                (document.language_and_locale_v2[1].entries) && (model.languagesKnown = document.language_and_locale_v2[1].entries.map((item: {"data": {"value": string}}) => DecodingUtils.decodeObject(item.data.value)));
                (document.language_and_locale_v2[2].entries[0].data?.value) && (model.favouriteLanguage = DecodingUtils.decodeObject(document.language_and_locale_v2[2].entries[0].data.value));
    ...
    static async parsePagesRecommended(data: Buffer): Promise<PagesRecommendedFB | undefined> {
        try {
            ...
            (document) && (document.recommended_pages_v2 && document.recommended_pages_v2.length > 0) && (modelPages.list = document.recommended_pages_v2.map((value: any) => {
                ...
                (value.name) && (model.name = DecodingUtils.decodeObject(value.name));
                (value.timestamp) && (model.date = new Date(1000 * value.timestamp));
                (value.url) && (model.url = DecodingUtils.decodeObject(value.url));
                return model;
            }));
                ...
                if(document.recently_viewed[0] && document.recently_viewed[0].children) {
                    if (document.recently_viewed[0].children[0] && document.recently_viewed[0].children[0].entries) {
                        modelRecentlyViewed.timeSpentOnPageVideos = document.recently_viewed[0].children[0].entries.map((entry: any) => {
                            ...
                            (entry.data?.name) && (model.name = DecodingUtils.decodeObject(entry.data.name));
                            (entry.data?.uri) && (model.uri = DecodingUtils.decodeObject(entry.data.uri));
                            (entry.data?.watch_time) && (model.watchTimeInSeconds = DecodingUtils.decodeObject(entry.data.watch_time));
                            return model;
                        });
                    }
                    ...
                    if (document.recently_viewed[0].children[2] && document.recently_viewed[0].children[2].entries) {
                        modelRecentlyViewed.timeSpentOnSingleVideo = document.recently_viewed[0].children[2].entries.map((entry: any) => {
                            ...
                            (entry.data?.name) && (model.name = DecodingUtils.decodeObject(entry.data.name));
                            (entry.data?.uri) && (model.uri = DecodingUtils.decodeObject(entry.data.uri));
                            (entry.data?.watch_position_seconds) && (model.watchTimeInSeconds = DecodingUtils.decodeObject(entry.data.watch_position_seconds));
                            return model;
                        });
                    }
                }
    ...
    static async parseYourPosts(data: Buffer): Promise<YourPostsFB | undefined> {
        try {
            ...
            (document) && (modelPosts.list = document.map((item: any) => {
                ...
                (item.title) && (model.title = DecodingUtils.decodeObject(item.title));
                (item.data && item.data.post) && (model.post = DecodingUtils.decodeObject(item.data.post));
    ...
    static async parseFriendRequestsSent(data: Buffer): Promise<FriendRequestsSentFB | undefined> {
        try {
            ...
            (document) && (modelPosts.list = document.sent_requests_v2.map((item: any) => {
                ...
                (item.timestamp) && (model.date = new Date(1000 * item.timestamp));
                (item.name) && (model.name = DecodingUtils.decodeObject(item.name));
                return model;
            }));
            return modelPosts.list.length > 0 ? modelPosts : undefined;
        } catch (error) {
    ...
    static async parseYourTopics(data: Buffer): Promise<YourTopicsFB | undefined> {
        try {
            ...
            if (document && document.inferred_topics_v2) {
                modelPosts.list = document.inferred_topics_v2.map((item: string) => DecodingUtils.decodeObject(item));
            }
            return modelPosts.list.length > 0 ? modelPosts : undefined;
        } catch (error) {
    ...
    static async parseAdsInterests(data: Buffer): Promise<AdsInterestsFB | undefined> {
        try {
            ...
            if (document && document.topics_v2) {
                modelPosts.list = document.topics_v2.map((item: string) => DecodingUtils.decodeObject(item));
            }
            return modelPosts.list.length > 0 ? modelPosts : undefined;
        } catch (error) {
        ...
        try {
            ...
            if (document) {
                modelInfo.list = document.lead_gen_info_v2.map((item: any) => {
                    ...
                    (item.label) && (model.label = DecodingUtils.decodeObject(item.label));
                    (item.value) && (model.value = DecodingUtils.decodeObject(item.value));
                    return model;
                });
            }
            ...
            if (document && document.visited_things_v2) {
                ...
                for (let i = 0; i < document.visited_things_v2.length-1; i++) {
                    ...
                    (document.visited_things_v2[i].entries) && (modelList.list = document.visited_things_v2[i].entries.map((item: any) => {
                        ...
                        (item.timestamp) && (model.date = new Date(1000 * item.timestamp));
                        (item.data && item.data.name) && (model.name = DecodingUtils.decodeObject(item.data.name));
                        (item.data && item.data.uri) && (model.uri = DecodingUtils.decodeObject(item.data.uri));
                        return model;
                    }));
