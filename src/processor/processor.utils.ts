...
export function monthIsInRange(
    ...
    if (match && match[1] && match[2]) {
        const month: number = parseInt(MonthsFull[match[2] as any], 10);
        if (month) {
            //if 2017_APRIL then returns 1st APRIL 2017
            const date: Date = new Date(
                Date.UTC(parseInt(match[1], 10), month - 1, 1),
            );
            if (daysDifference(date) < timeIntervalDays) {
                return true;
            }
        }
    }
